...
// ============================================================
// 記事ビューア（戻る/進む付き）+ プロキシ
//   OBSブラウザソースには /news/view?u=<記事URL> を表示させる。
//   記事は /news/proxy 経由で配信し、X-Frame-Options を回避。
//   記事内リンクも proxy 経由に書き換えるので iframe 内 history.back() で戻れる。
// ============================================================
const NEWS_UA = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0 Safari/537.36';

app.get('/news/view', (req, res) => {
  const u = req.query.u || '';
  const inner = '/news/proxy?u=' + encodeURIComponent(u);
  res.set('Content-Type', 'text/html; charset=utf-8');
  res.send(`<!doctype html><html lang="ja"><head><meta charset="utf-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<style>
  html,body{margin:0;height:100%;background:#fff;font-family:'Yu Gothic UI','Noto Sans JP',sans-serif;}
  #bar{position:fixed;top:0;left:0;right:0;height:46px;background:#1a1a1a;display:flex;align-items:center;gap:8px;padding:0 10px;z-index:2147483647;box-shadow:0 2px 8px rgba(0,0,0,.4);}
  #bar button{background:#e53e3e;color:#fff;border:none;border-radius:6px;padding:9px 16px;font-size:15px;font-weight:700;cursor:pointer;line-height:1;}
  #bar button.sub{background:#333;}
  #fr{position:absolute;top:46px;left:0;right:0;bottom:0;width:100%;height:calc(100% - 46px);border:0;background:#fff;}
</style></head><body>
<div id="bar">
  <button onclick="nav('back')">← 戻る</button>
  <button class="sub" onclick="nav('forward')">進む →</button>
  <button class="sub" onclick="nav('reload')">⟳ 更新</button>
</div>
<iframe id="fr" src="${inner}"></iframe>
<script>
  function nav(a){
    try{
      var w=document.getElementById('fr').contentWindow;
      if(a==='back')w.history.back();
      else if(a==='forward')w.history.forward();
...
app.get('/news/proxy', async (req, res) => {
  const target = req.query.u;
  if (!target || !/^https?:\/\//i.test(target)) return res.status(400).send('bad url');
  try {
    const r = await safeFetch(target, { headers: { 'User-Agent': NEWS_UA, 'Accept': 'text/html,application/xhtml+xml' } });
    const ct = r.headers.get('content-type') || '';
    if (!/text\/html/i.test(ct)) { return res.redirect(target); }
    let html = await r.text();
    const base = target;
    // <head> 直後に <base> を挿入（相対パスのCSS/画像を救済）
    if (/<head[^>]*>/i.test(html)) {
      html = html.replace(/<head([^>]*)>/i, '<head$1><base href="' + base + '">');
    } else {
      html = '<base href="' + base + '">' + html;
    }
    // <a href> を proxy 経由に書き換え（http/https のみ）
    html = html.replace(/<a\s+([^>]*?)href=["']([^"']+)["']/gi, (m, pre, href) => {
      let abs;
      try { abs = new URL(href, base).href; } catch (e) { return m; }
      if (!/^https?:/i.test(abs)) return m;
      return '<a ' + pre + 'href="/news/proxy?u=' + encodeURIComponent(abs) + '"';
    });
    res.removeHeader('X-Frame-Options');
    res.set('Content-Type', 'text/html; charset=utf-8');
    res.send(html);
  } catch (e) {
    console.error('[news/proxy]', e.message);
